...
def sub_time(u_key, subway_code, upDTCode): #지하철 시간표 검색 함수
    lim_t = 0
    day_time = day_t()
    real_time = KST.strftime(k_time)#어느 국가에 위치한 서버든지 상관없이 한국 표준시로 사용하도록.
    time_list = []
    if int(real_time) > 150000:
        pageNumber = 10
        try_n = 20
    else :
        pageNumber = 1
...
def subway_timetable(time_l, sub_name):#사용자에게 전송할 타임 테이블, sub_time(...) 에서 구한 시간표를 사용자에게 제공하기 위해 정비
    ...
    return result_s
def search_line_name(start_sub_name, start_line_name):
    ...
    if tmp_list == []:
        ...
        user_answer_l = [{"messageText": "명령어","action": "message","label": "명령어 보기"},{"messageText": "정자역","action": "message","label": "예시 1번"},{"messageText": "서울","action": "message","label": "예시 2번"},{"messageText": "판교역","action": "message","label": "예시 3번"}]
...
def search_time_table(time_sub_name, time_line_name):
    ...
    try :
        ...
        result_l += sub_time(u_key_decode, subway_code, 'U')#사용자가 원하는 지하철 상행 시간표
        result_l += sub_time(u_key_decode, subway_code, 'D')#사용자가 원하는 지하철 하행 시간표
